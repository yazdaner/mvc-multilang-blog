            ...
            <div class="col-lg-9">
                <div class="content-post bg-white mt-5 p-md-5 p-3 py-5">
                    ...
                    <div class="image-post">
                        ...
                        <img class="img-fluid" src="{{ env('NEWS_BANNER_IMAGES_PATH') . $news->banner }}" alt=""
                    ...
                    <div class="main-content-post">
                        ...
                        <div class="share-news d-flex align-items-center">
                            ...
                            <div class="social-share ms-5">
                                <a href="https://telegram.me/share/url?url={{ URL::to('/') . '/news/' . $news->slug }}"><i
                                        class="lh-0 bi bi-telegram"></i></a>

                                <a href="https://whatsapp://send/?text={{ URL::to('/') . '/news/' . $news->slug }}"><i
                                        class="lh-0 bi bi-whatsapp"></i></a>
                ...
                <div class="content-post bg-white my-5 p-md-5 p-3 py-4" id="response">
                    <h4 class="fw-bold">{{ $news->getCommentsCount() }} {{ __('Comments') }} </h4>
                    <hr>
                    ...
                    @auth
                        <div class="mb-5">
                            ...
                            <form action="{{ route('news.comment.store', $news) }}" method="post">
                                @csrf
                                ...
                                <button class="btn btn-danger"
                                    onclick="event.preventDefault(); setNull()">{{ __('cancel') }}</button>
                            </form>
                            <hr class="my-4">
                    ...
                    @else
                        <div class="alert alert-secondary mb-5" role="alert">
                            {{ __('Please') }} <a href="{{ route('login') }}">{{ __('login') }}</a>
                            {{ __('to news a comment') }}
                        </div>
                    @endauth
                ...
                <div class="info-post bg-white mt-md-5">
                    ...
                    <div class="d-flex align-items-center justify-content-start">
                        <div class="me-3">
                            @php
                                if ($news->user->avatar) {
                                    $avatar = asset(env('USER_IMAGES_PATH') . $news->user->avatar->image);
                                } else {
                        ...
                        <div class="">
                            <h5 class="fw-bold">{{ $news->user->name }}</h5>
                            <p>Admin</p>
                        </div>
                    </div>
                    <hr>
